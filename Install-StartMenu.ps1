<#
.SYNOPSIS
This script configures the Start Menu and ensures it applies only once per user.
.DESCRIPTION
The script works by performing the following steps:
1. Creates RunOnce Key for the first deployed user to run the VBS script that executes the PS script hidden.
2. Creates a Scheduled-Task to check if the Start Menu has been applied, this runs as a "logon-script" and will run for all users that log on to the device.
3. Creates a registry entry to indicate if the Start Menu was successfully configured.
4. Logs actions to Windows\Temp.
#>
...
# Required variables
$Destination = "$env:SystemDrive\ProgramData\AutoPilotConfig\StartMenu"
$scriptPath = "$Destination\RunHidden.vbs"
$Source = $PSScriptRoot

# Log File Info
$Now = Get-Date -Format "yyyyMMdd-HHmmss"
$LogPath = "$ENV:WINDIR\Temp\ImportStartMenu_$Now.log"
...
Function CleanUpAndExit() {
    ...
    $StoreResults = "\krevoit\StartMenu" # Change this to something that fits your organization.
...
function Set-RunKey {
    ...
    foreach ($userPath in (Get-ChildItem "Registry::HKEY_USERS\" | Where-Object { $_.Name -notmatch '_Classes|S-1-5-18|S-1-5-19|S-1-5-20|\.DEFAULT' })) {
        ...
        try {
            ...
            [Microsoft.Win32.Registry]::SetValue($RunPath, "StartMenuImport", "wscript.exe `"$scriptPath`"", [Microsoft.Win32.RegistryValueKind]::String)
...
function New-ScheduledTaskAllUsers {
    ...
    try {
        ...
        Register-ScheduledTask -Action $action -Trigger $trigger -TaskName "SetStartMenu" -Description "Set Start Menu at LogOn" -Principal $principal
